...
class Util {
    ...
    /**
     * 使用闭包（php>=5.3）对数组中的对象或数组根据指定字段进行排序，
     * 若服务器版本低于5.3请勿开启此函数否则会报错
     ...
     * @return int  两点间的距离
     */
    public static function getDistance($lng1, $lat1, $lng2, $lat2, $decimal = 0)
    {
...
//    }
    ...
    public static function getServerFileUrl($path)
    {
        ...
        return self::getServerUrl() . ltrim($path, '.');
    }
    ...
    {
        return preg_match("/^1[34578]{1}[0-9]{9}$/", $phone);
    }
    ...
    public static function filterIdList($ids, $to_string = false)
    {
        ...
        return $to_string ? implode(',', $ids) : $ids;
    }
